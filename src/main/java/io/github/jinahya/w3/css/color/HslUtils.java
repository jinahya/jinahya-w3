...
public final class HslUtils {
    ...
    private static double requireValidHue(final double hue) {
    ...
    private static double requireValidSaturation(final double saturation) {
    ...
    private static double requireValidLightness(final double lightness) {
    ...
    public static <T extends DoubleBuffer> T hslToRgbBuffer(
            final double hue, final double saturation, final double lightness, final T buffer) {
        requireValidHue(hue);
        requireValidSaturation(saturation);
        requireValidLightness(lightness);
        ...
        return (T) buffer.put(r).put(g).put(b);
    }
     ...
     * @see #hslToRgbBuffer(double, double, double, DoubleBuffer)
     * @see <a href="https://www.w3.org/TR/css-color-3/#hsl-color">4.2.4. HSL color values</a> (CSS Color Module Level
     * 3, W3C Recommendation, 18 January 2022)
     */
    // https://www.w3.org/TR/css-color-3/#hsl-color
    // https://www.rapidtables.com/convert/color/hsl-to-rgb.html
    // https://colordesigner.io/convert/hsltorgb
    public static <R> R hslToRgbFunction(final double hue, final double saturation, final double lightness,
        ...
        final var buffer = hslToRgbBuffer(hue, saturation, lightness, DoubleBuffer.allocate(3));
        return function
                .apply(buffer.get(0))
                .apply(buffer.get(1))
                .apply(buffer.get(2))
    ...
    // https://www.rapidtables.com/convert/color/hsl-to-rgb.html
    // https://colordesigner.io/convert/hsltorgb
    public static double[] hslToRgbArray(final double hue, final double saturation, final double lightness) {
        requireValidHue(hue);
        requireValidSaturation(saturation);
        requireValidLightness(lightness);
        return hslToRgbBuffer(
                hue,
                saturation,
                lightness,
                DoubleBuffer.allocate(3)
